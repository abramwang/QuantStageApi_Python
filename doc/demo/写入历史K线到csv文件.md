...
mapi = PT_QuantApi_Python36.PT_QuantApi(mspi, True, "Td_Real", "MD_Real")  #此处设置，创建的是交易的实盘环境，以及行情的实时环境    交易生产环境在账号有权限的情况下将Td_Real替换成Td_SimulateTd即可
...
用户登录成功后，若需**订阅历史K线行情**，则需先对之进行**参数赋值**操作。
...
```python
 ...
 SubType = ["kline"]                # 列表
 CycType = ["day"]                  # 列表
 pSubWindCode = ["600000.SH","002003.SZ"]       # 列表，空值表示请求全市场代码
...
处理完成后，系统触发通知K线数据回调函数，回调函数收到相应的行情信息。
...
class DataCallBack(QuantCallBack):
    ...
    def __init__(self):
        ...
        #创建csv文件
        self.Historyfile = open('HistoryKLine.csv', 'w+',newline='')#newline=''说明是csv文件类型
        self.Historyewriter=csv.writer(self.Historyfile)
        #写入表格头
        self.Historyewriter.writerow(['行情类型','证券代码','时间','开盘价','最高价','最低价','今收价','昨收价','涨停价','跌停价','成交数量','成交金额','周期类型'])
    ...
    def OnRspSubQuote(self, pData):
        print ("OnRspSubQuote",pData)
        self.Historyfile.close()#关闭文件
        pass
    #缓存推送结束,之后的数据为实时推送行情数据
    ...
    def OnRtnKLine(self, pKline):
        writedata=[pKline["szType"],pKline["szWindCode"],pKline["szDatetime"],pKline["nOpen"],pKline["nHigh"],pKline["nLow"],pKline["nClose"],pKline["nPreClose"],pKline["nHighLimit"],pKline["nLowLimit"],pKline["nVolume"],pKline["nTurover"],pKline["szCycType"]]
        print("writedata",writedata)#打印
        self.Historyewriter.writerow(writedata)#写入文件
        pass
...
def main():
    ...
    mapi.ReqSubQuote(3, ["kline"], ["day"], ["600000.SH","002003.SZ"], "2017-05-24 8:30:00", "2017-07-24 24:00:00")
    mapi.Run()
