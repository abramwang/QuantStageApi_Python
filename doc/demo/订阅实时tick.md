...
创建**交易回调类**，继承父类的回调方法，用于接收回调信息。比如，登录成功后，用户会接收到对应的登录回调信息；订阅实时tick成功后，会收到对应的订阅实时tick回调信息。
...
class DataCallBack(QuantCallBack):
    ...
    def OnRtnMarket(self, pMarket):           # 通知个股行情
        ...
        pass
    def OnRtnOrder(self, pOrder):             # 通知逐笔委托
        pass
    def OnRtnOrderQueue(self, pOrderQueue):   # 通知委托队列
        print ("OnRtnOrderQueue",pOrderQueue)
        pass
    def OnRtnTransaction(self, pTransaction): # 通知逐笔成交
        pass
...
用户登录成功后，若需**订阅实时tick**，则需先对之进行**参数赋值**操作。
...
收到指令后，系统触发行情、逐笔委托、委托队列、逐笔成交数据等相关回调函数，回调函数收到相应的行情信息。
